...
def GetMel( wav_fd, fe_fd, n_delete ):
    ...
    for na in names:
        ...
        X_m=np.log(X_m+1.0e-9) # loge for better hearing perception 为了更好的听觉感知
...
###
# get chunk data, size: N*agg_num*n_in 获取块数据，大小：
def GetAllData( fe_fd, agg_num, hop, fold , scaler,fea_dim):
    ...
    for li in lis:
        ...
        len_X, n_in = X.shape
        #print X.shape
        #padding zeros in the left begining with (agg_num-1)/2 zeros: 从左边开始填充（agg_num-1）/ 2个零：
        X = np.concatenate( ( np.zeros(((agg_num-1)/2, n_in))+ (-50.0), X  ) )  ### due to log, so -50.0
        #padding zeros in the right end with (hop-1)/2 zeros:用（hop-1）/ 2个零填充右端的零
...
###
# create an empty folder 创建一个空的文件夹
def CreateFolder( fd ):
