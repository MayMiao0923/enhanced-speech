...
# truncate seq or pad with 0, input can be list or np.ndarray截断seq或pad为0，输入可以是list或np.ndarray
# the element in x should be ndarray, then pad or trunc all elements in x to max_len
# x中的元素应为ndarray，然后将x中的所有元素填充或截断为max_len
# type: 'post' | 'pre'
...
def mat_2d_to_3d_paddingzeros( X, agg_num, hop ):
    # pad to at least one block 至少填充一块
    len_X, n_in = X.shape
    if ( len_X < agg_num ):
        X = np.concatenate( ( X, np.zeros((agg_num-len_X, n_in)) ) )
    ...
    ### replaced by zero padding when reading X读取X时替换为零填充
    ##padding zeros in the left begining with (agg_num-1)/2 zeros: 左边的填充零开始于（agg_num-1）/ 2个零：
...
# sparse label to categorical label 稀疏标签到分类标签
# x is 1-dim ndarray
