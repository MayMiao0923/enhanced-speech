...
基于DNN的语音增强训练脚本，基于keras进行清洁映射验证
Author: YONG XU
Date: 2017.12.21
"""
import sys
...
# 调整数据大小以适应CNN。 尺寸：（batch_num * color_maps * height * weight）
def reshapeX( X ):
    N = len(X)
...
agg_num = 7        # concatenate frames 连接框架
hop = 1            # step_len  步长

n_out = 257

fold = 1           # 1 will be the test set;;; 0 will be the training set 1将是测试集;;; 0将是训练集
fea_dim=257
...
with open('./tr_norm.pickle', 'rb') as handle:
    scaler = pickle.load(handle,encoding='bytes')
...
### target fea, the middle frame  目标fea，中间框架
tr_y=tr_X[:,((agg_num-1)/2)*fea_dim:((agg_num-1)/2+1)*fea_dim] ### arrive at 4*fea_dim-1 到达4 * fea_dim-1
...
### build model by keras 由keras建立模型
...
dnn = Dense(fea_dim,activation='linear')(dnn)   ### the middle frame out 中间框架
