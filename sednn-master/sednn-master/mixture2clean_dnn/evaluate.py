"""
Summary:  Calculate PESQ and overal stats of enhanced speech. 计算增强语音的PESQ和总体统计数据。
...
def plot_training_stat(args):
    """Plot training and testing loss. 绘制训练和测试损失
...
def calculate_pesq(args):
    """Calculate PESQ of all enhaced speech. 计算所有增强语音的PESQ
    ...
    # Remove already existed file. 删除已存在的文件
    ...
    for (cnt, na) in enumerate(names):
        ...
        # Call executable PESQ tool. 调用可执行的PESQ工具
        cmd = ' '.join(["./pesq", speech_path, enh_path, "+16000"])
        os.system(cmd)        
        
        
def get_stats(args):
    """Calculate stats of PESQ. 计算PESQ的统计数据。
    """
