"""
Summary:  Train, inference and evaluate speech enhancement. 训练，推理和评估语音增强
...
def eval(model, gen, x, y):
    ...
    # Inference in mini batch. mini batch 的推断
    for (batch_x, batch_y) in gen.generate(xs=[x], ys=[y]):
        pred = model.predict(batch_x)
        pred_all.append(pred)
        y_all.append(batch_y)
        
    # Concatenate mini batch prediction.  连接迷你批次预测
...
def train(args):
    ...
    # Directories for saving models and training stats 保存模型和培训统计数据的目录
    ...
    # Print loss before training. 训练之前，打印损失
    iter = 0
    ...
    # Save out training stats. 保存训练统计数据
    stat_dict = {'iter': iter, 
                    'tr_loss': tr_loss, 
                    'te_loss': te_loss, }
    ...
    for (batch_x, batch_y) in tr_gen.generate(xs=[tr_x], ys=[tr_y]):
        ...
        # Validate and save training stats. 验证并保存训练统计信息
        if iter % 1000 == 0:
            tr_loss = eval(model, eval_tr_gen, tr_x, tr_y)
            te_loss = eval(model, eval_te_gen, te_x, te_y)
            print("Iteration: %d, tr_loss: %f, te_loss: %f" % (iter, tr_loss, te_loss))
            
            # Save out training stats. 保存训练统计信息
            stat_dict = {'iter': iter, 
                         'tr_loss': tr_loss, 
                         'te_loss': te_loss, }
            stat_path = os.path.join(stats_dir, "%diters.p" % iter)
