"""
Summary:  Recover spectrogram to wave. 将频谱图恢复为波形。
...
def recover_wav(pd_abs_x, gt_x, n_overlap, winfunc, wav_len=None):
    """Recover wave from spectrogram. 从频谱图中恢复波
    If you are using scipy.signal.spectrogram, you may need to multipy a scaler
    to the recovered audio after using this function.
    如果您使用scipy.signal.spectrogram，则可能需要在使用此函数后将标量乘以恢复的音频。
     For example,recover_scaler = np.sqrt((ham_win**2).sum())
...
def real_to_complex(pd_abs_x, gt_x):
    """Recover pred spectrogram's phase from ground truth's phase. 从实况的相位恢复pred谱图的相位
...
def half_to_whole(x):
    """Recover whole spectrogram from half spectrogram. 从半频谱图中恢复整个频谱图。
    """
...
def deframesig(frames,siglen,frame_len,frame_step,winfunc=lambda x:numpy.ones((x,))):    
    """Does overlap-add procedure to undo the action of framesig.重叠添加过程是否撤消了frameig的操作
