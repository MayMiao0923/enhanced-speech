...
def create_mixture_csv(args):
    """Create csv containing mixture information.  创建包含混合信息的csv。
    ...
    for speech_na in speech_names:
        ...
        # 对于训练数据，将每个语音与随机选择的放大率噪声混合
        if data_type == 'train':
            selected_noise_names = rs.choice(noise_names, size=magnification, replace=False)
        # For test data, mix each speech with all noises.对于测试数据，将每个语音与所有噪声混合。
        elif data_type == 'test':
            selected_noise_names = noise_names
        else:
            raise Exception("data_type must be train | test!")

        # Mix one speech with different noises many times. 将一个语音与不同的噪音混合多次。
...
def calculate_mixture_features(args):
    ...
    features to disk. 计算混合语音，语音和噪声音频的频谱图。 然后将功能写入磁盘。
    ...
    for i1 in xrange(1, len(lis)):
        ...
        # Read speech audio. 阅读语音音频。
        speech_path = os.path.join(speech_dir, speech_na)
        (speech_audio, _) = read_audio(speech_path, target_fs=fs)
        
        # Read noise audio. 阅读噪声音频。
        noise_path = os.path.join(noise_dir, noise_na)
        (noise_audio, _) = read_audio(noise_path, target_fs=fs)
        
        # Repeat noise to the same length as speech. 重复噪音到与语音相同的长度。
...
def get_amplitude_scaling_factor(s, n, snr, method='rms'):
    """Given s and n, return the scaler s according to the snr. 给定s和n，根据snr返回缩放器。
...
def additive_mixing(s, n):
    """Mix normalized source1 and source2. 混合规范化的source1和source2。
...
def mat_2d_to_3d(x, agg_num, hop):
    """Segment 2D array to 3D segments. 将2D阵列划分为3D段。
    """
    # Pad to at least one block. 填充至少一个块。
    len_x, n_in = x.shape
    if (len_x < agg_num):
...
def compute_scaler(args):
    """Compute and write out scaler of data. 计算并写出数据规模。
    """
    workspace = args.workspace
    data_type = args.data_type
    snr = args.snr
...
def scale_on_2d(x2d, scaler):
    """Scale 2D array data. 缩放2D数组数据
    """
    return scaler.transform(x2d)
    
def scale_on_3d(x3d, scaler):
    """Scale 3D array data. 缩放3D数组数据
    """
