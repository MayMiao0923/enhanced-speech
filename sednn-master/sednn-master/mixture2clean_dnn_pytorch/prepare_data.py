...
def write_features_to_hdf5(audio_dir, hdf5_path, data_type, audio_type, sample_rate, fft_size, hop_size, window_type):
    ...
    with h5py.File(hdf5_path, 'a') as hf:
        ...
        # Spectrogram of a song一首歌的谱图
        names = os.listdir(audio_dir)
        
        for na in names:
            
            # Extract spectrogram & raw audio frames 提取频谱图和原始音频帧
            ...
            # Write spectrogram & raw audio frames out to hdf5 将频谱图和原始音频帧写入hdf5
        ...
        # Write out bin_fin_indices to hdf5 写出bin_fin_indices到hdf5
        hf[data_type][audio_type].create_dataset(
            name='bgn_fin_indices', 
            data=bgn_fin_indices, 
            dtype=np.int32)
            
        # Write out energy_list 写出energy_list
        hf[data_type][audio_type].create_dataset(
            name='energies', 
            data=energy_list, 
            dtype=np.float32)
            
        # Write out name_list to hdf5 写出name_list到hdf5
        hf[data_type][audio_type].create_dataset(
            name='names', 
            data=name_list, 
            dtype='S64')
...
class DataLoader(object):
    ...
    def mix_speech_noise(self, speech_x, noise_x, speech_eng, noise_eng, snr):
        ...
        # Scale coefficient of noise 噪声的比例系数
...
class DFT(object):
    ...
    def flip(self, x):
        """Flip a tensor along the last dimension.  沿最后一个维度翻转张量。
        """
